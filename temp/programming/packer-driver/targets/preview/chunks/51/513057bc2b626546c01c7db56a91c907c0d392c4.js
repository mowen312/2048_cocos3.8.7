System.register(["cc"], function (_export, _context) {
  ...
  var _cclegacy, __checkObsolete__, __checkObsoleteInNamespace__, _decorator, Color, Component, Label, Sprite, tween, UITransform, Vec3, _dec, _class, _crd, ccclass, property, Grid;
  ...
  return {
    ...
    execute: function () {
      ...
      _cclegacy._RF.push({}, "9557160LmhBi5jya/nKJixm", "Grid", undefined);
      /*
      ...
      */
      ...
      // 注册组件，允许在编辑器中使用
      __checkObsolete__(['_decorator', 'AudioClip', 'Color', 'Component', 'Label', 'Node', 'Sprite', 'tween', 'UITransform', 'Vec3']);
      ...
      _export("Grid", Grid = (_dec = ccclass('Grid'), _dec(_class = class Grid extends Component {
        constructor() {
          ...
          this._value = 0;
          // 标记格子是否可被控制（预留功能）
          this._canContorl = false;
        }

        // 组件加载时调用（初始化）
        onLoad() {
          // 获取精灵组件引用（用于设置背景颜色）
          this._sprite = this.node.getComponent(Sprite); // 获取UI变换组件引用（用于设置大小）

          this._uiTransform = this.node.getComponent(UITransform); // 获取子节点"GridNumber"上的标签组件（用于显示数字）

          var gridNumberNode = this.node.getChildByName("GridNumber");
        ...
        setValue(value, animate) {
          ...
          // 记录旧值，用于判断是否需要播放动画
          var oldValue = this._value; // 更新当前值

          this._value = value; // 如果标签组件存在，设置显示文本（0不显示）
          ...
          if (value > 0) {
            var digitCount = value.toString().length; // 获取数字的位数
          ...
          this.UpdateColor(); // 动画逻辑：根据数值变化判断是否播放动画
        ...
        UpdateColor() {
          // 如果精灵组件不存在，直接返回
          if (!this._sprite) return; // 定义不同数值对应的背景颜色映射表
          ...
          this._sprite.color = colorMap[this._value] || colorMap[0]; // 设置文字颜色（数值>=8用白色，否则用黑色，增强对比度）
        ...
        playMoveAnimation(targetPos, duration) {
          if (duration === void 0) {
            duration = 2.0;
          }
        ...
        playSpawnAnimation() {
          // 初始状态：缩放到0（不可见）
          this.node.setScale(new Vec3(0, 0, 1)); // 动画序列：先放大到1.1倍（0.15秒），再缩小到1倍（0.1秒）
...
//# sourceMappingURL=513057bc2b626546c01c7db56a91c907c0d392c4.js.map