    ...
    execute: function () {
      ...
      _export("Swipe", Swipe = (_dec = ccclass('Swipe'), _dec2 = property(Node), _dec3 = property(AudioClip), _dec(_class = (_class2 = class Swipe extends Component {
        ...
        onTouchMove(event) {
          ...
          const deltaY = currentPos.y - this.startPos.y; // 触摸移动日志，打印当前位置和偏移量
        ...
        onTouchEnd(event) {
          ...
          const distance = Math.sqrt(deltaX * deltaX + deltaY * deltaY); // 触摸结束基础信息日志
          ...
          let direction = SwipeDirection.None; // 滑动距离达标且有移动行为，才判定为有效滑动
